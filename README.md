...
1. **Add a parameter** from the status-bar param view (activity bar) or the
   ...
   file, type, id, and values; advanced options are optional. Once the parameter is
   described (type, id, and — for an array — value shape), it offers to either guide
   you through the values or **insert a complete example to edit in JSON** — handy for
   the named-output shape, which is quicker to tweak in the file than to enter prompt
   by prompt.
...
An array parameter's value shape is chosen in the wizard after the id, or by
